...
Author(s): Kurt Riitters
 ...
 spatcon.c is in the public domain and distributed in the hope that it will be useful.
 ...
 * gcc -std=c99 -m64 -O2 -Wall -lm spatcon.c -o spatcon
----------
2 July 2018.
...
October 1998.
   ...
   (now 2.6) and linux (redhat 5.0-5.1).
...
Added patch_window routines, 1/95
...
Added re-coding of input pixel values 12/94
     If zZ is specified, then this program looks for a file called
     '<arg1>.rec' in the same directory as <arg1>.  The file contains
     the recoding rules.

Added lpt generator, 12/94
...
Classic Mode Usage: spatconv <infile> <outfile> <parameterfile>
	 ...
	 Use as many lines as needed.
				...
				p or P = output three additional "Px" files for the lptmaker (map rule 6)
...
float *mat_outfloat;
/* the pointers to three additional lpt maps have to be visible everywhere */
static FILE *mapP2_1, *mapP2_2, *mapP2_3;   /* for enhanced lptmaker*/
...
struct run_parameters {
	...
	long int threemaps;      /* 0 = don't output extra three maps, 1 = output three additional maps for lptmaker map rule 6 or 7 */
};                              /* Default parameter values */
struct run_parameters parameters = {0,0,0,1,0,0,0,0,0};
...
int main(int argc, char **argv)
{
	...
	static char filename_P2_1[100], filename_P2_2[100], filename_P2_3[100];
	...
	if(argc == 1) { /* guidos mode */
		...
		strcpy(filename_P2_1, "p2_1.bsq");
		strcpy(filename_P2_2, "p2_2.bsq");
		strcpy(filename_P2_3, "p2_3.bsq");
	}
	...
	if(ret_val != 0) {
		printf("\nSpatconv:Error reading run from file %s.\n", filename_par);
		exit(7);
	}
	fclose(parfile);
	/* If re-coding pixels ... */
	if(parameters.recode == 1) {
		...
		while(fscanf(recfile, "%ld %ld", &vold, &vnew) == 2) {
			...
			if(parameters.map_rule == 6) { /* doing LPT's... */
				if( (vnew < 0) || (vnew > 3) ) {
					printf("Oops, recode to 0,1,2,3 only, for LPT's.\n");
					fclose(recfile);
					exit(5);
				}
			}
	...
	printf("Run parameters are: m%ld w%ld r%ld h%ld a%ld b%ld z%ld f%ld p%ld\n",
	       ...
	       parameters.recode, parameters.outfloat, parameters.threemaps);
	...
	*/
	...
	/* open three more binary output files if threemaps option is set, this can only be for lptmaker */
	if( (parameters.map_rule == 6) || (parameters.map_rule == 7) ) { 
		if( parameters.threemaps == 1) { /* want three additional maps */
			if( (mapP2_1 = fopen(filename_P2_1, "wb") ) == NULL) {
				printf("\nSpatconv:Error opening %s\n", filename_P2_1);
				exit(3);
			}		
			if( (mapP2_2 = fopen(filename_P2_2, "wb") ) == NULL) {
				printf("\nSpatconv:Error opening %s\n", filename_P2_2);
				exit(3);
			}		
			if( (mapP2_3 = fopen(filename_P2_3, "wb") ) == NULL) {
				printf("\nSpatconv:Error opening %s\n", filename_P2_3);
				exit(3);
			}		
		}
	...
	fscanf(sizfile,"%s %ld", header_line, &nrows_in);
	fscanf(sizfile,"%s %ld", header_line, &ncols_in);
	fclose(sizfile);
...
{
	static long int okay, counter, n_colors_in_image, edge_freq,
	       color_freq, array_length, preserve_original_colors, t1, t2;
	static long int n_rows, n_cols, buff_b, temp_int,
	...
	static unsigned char map_value;
	static float map_value_float;
	static long int in_to_out[256], out_to_in[256];
	static long int *freq_ptr;
	static unsigned char *mat_temp;
	...
	/* all of the references to grain size are for when the step size could be somththing else */
...
unsigned char Freq_Filters(long int color_freq, long int edge_freq, long int *freq_ptr, long int n_colors_in_image,
                           long int mapping_rule, long int handle_missing, long int code_1, long int code_2)
{
	static unsigned char map_val, mv1;
	static long int index, start, temp_int, temp_int2, ind1, ind2, n_colors_in_window;
	static float temp_float, temp_float_2, temp_float_3;
	static float p_agr, p_for, p_dev;
	switch(mapping_rule) {
	...
	case 52: /* return Simpson evenness, diversity/max diversity */
		...
		temp_float = 0;
		for(index = start; index < (n_colors_in_image + 1); index++) {
			temp_float_3 = (*(freq_ptr + index)) * temp_float_2;
		...
		if(mapping_rule == 52) { /* change diversity to evenness */
			temp_float_3 = 0;  /*counter of number of colors in window*/
			for(index = start; index < (n_colors_in_image + 1); index++) {
				if( (*(freq_ptr + index)) > 0) {
					temp_float_3++;
				}
			}
			if(temp_float_3 == 1) { /* only one color */
				map_val = 255;
				break;
			} else {
				temp_float = temp_float / (1.0 - (1.0 / temp_float_3));
				temp_int = (temp_float * 254.) + 1;
				map_val = temp_int;
				break;
			}
	...
	case 53: /* return shannon evenness */
		...
		if(temp_float_3 == 1) { /* only one color */
			...
			temp_float = (-1.0 * temp_float) / log(temp_float_3);
			temp_int = (temp_float * 254.) + 1;
			map_val = temp_int;
			break;
		}
	...
	case 71: /* return angular second moment, with regard to order*/
		...
		if(handle_missing == 1) {  /*missing not included*/
			temp_float_3 = 0.;  /* will be the divisor */
			...
			for(ind1 = 1; ind1 <= n_colors_in_image; ind1++) {
				for(ind2 = 1; ind2 <= n_colors_in_image; ind2++) {
					temp_int = (ind1 * (n_colors_in_image + 1)) + ind2;
					temp_float_2 = (*(freq_ptr + temp_int)) / temp_float_3;
					temp_float += temp_float_2 * temp_float_2;
				}
			}
			temp_int = (temp_float * 254.) + 1;
	...
	case 72: /* return Simpson edge-type evenness, or simpson contagion */
		...
		if(handle_missing == 2) {  /*missing included, why would anyone do this*/
			...
			temp_float_2 = (1.0 - (1.0 / (n_colors_in_window * n_colors_in_window)));
	...
	case 73: /* return shannon edge-type evenness, ie contagion */
		...
		if(handle_missing == 2) {  /*missing included, why would anyone do this*/
			...
			temp_float = -1.0 * temp_float;
			temp_float_2 = 2.0 * log(n_colors_in_window);
			temp_float = 1.0 - (temp_float / temp_float_2);
			temp_int = (temp_float * 254.) + 1;
			map_val = temp_int;
			break;
		...
		if(handle_missing == 1) {  /*missing not included*/
			...
			for(ind1 = 1; ind1 <= n_colors_in_image; ind1++) {
				for(ind2 = 1; ind2 <= n_colors_in_image; ind2++) {
					temp_int = (ind1 * (n_colors_in_image + 1)) + ind2;
					temp_float_3 = temp_float_3 + (*(freq_ptr + temp_int));
				}
			}
			/* return missing if there are no non-missing edges */
			if(temp_float_3 == 0.0) {
	...
	case 74: /* return sum of diagonal of adjacency matrix */
		...
		if(handle_missing == 1) {  /*missing not included*/
			temp_float_2 = 0.;  /* will be the divisor */
			...
			/* and the proportion is... */
			temp_float = temp_float / temp_float_2;
			temp_int = (temp_float * 254.) + 1;
			map_val = temp_int;
			break;
		}
	...
	case 75: /* return proportion of local edges which involve code_1 */
		...
		if(handle_missing == 1) {  /*missing not included*/
			...
			/* the diagonal cell has been counted twice, so subtract one of them */
			temp_int = (parameters.code_1 * (n_colors_in_image + 1)) + parameters.code_1;
			temp_float = temp_float - (*(freq_ptr + temp_int));
	...
	/* handle writing three more maps for lptmaker if requested */
	if( ( (parameters.map_rule == 6) || (parameters.map_rule == 7) ) && (parameters.threemaps == 1)) {
		if(map_val == 0) { /* if map_val was set to zero in the lptmaker above, then the window was missing and map_val should be zero on all three maps */
			if(fwrite(&map_val, 1, 1, mapP2_1) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
			if(fwrite(&map_val, 1, 1, mapP2_2) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
			if(fwrite(&map_val, 1, 1, mapP2_3) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
		}
		if(map_val != 0) { /* calculate three new byte values for  pa pf pd and write them, but don't change map_val*/
			temp_int = (p_agr * 254.) + 1;
			mv1 = temp_int;
			if(fwrite(&mv1, 1, 1, mapP2_1) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
			temp_int = (p_for * 254.) + 1;
			mv1 = temp_int;
			if(fwrite(&mv1, 1, 1, mapP2_2) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
			temp_int = (p_dev * 254.) + 1;
			mv1 = temp_int;
			if(fwrite(&mv1, 1, 1, mapP2_3) != 1 ) {printf("\nSpatconv: error writing lptmaker output file.\n");	exit(8); }
		}
	}
	return(map_val);
}
