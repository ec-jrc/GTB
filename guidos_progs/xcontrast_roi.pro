...
PRO Xcontrast_ROI_IMAGEWINDOWKILLED, imageWindowID
...
ENDIF
END
; of Xcontrast_ROI_IMAGEWINDOWKILLED  ***************************************



PRO Xcontrast_ROI_HISTOPLOT, image, Binsize = binsize, Reverse_Indices = r, $
...
END
;  of Xcontrast_ROI_HISTOPLOT   *********************************************


PRO XCONTRAST_ROI_done, event
;; return the contrast stretched image to the main program
...
result = 0 & xc_top = 0 & xc_max = 0 & xc_min = 'a' & mask = 0b
CASE  strlowCase(button_set) OF
   'accept': BEGIN
      ;;   * info_xstr.contrast_image = $
      ;;    BytScl( * info_xstr.image, Top = info_xstr.top,  $
      ;;            Max = info_xstr.maxThresh, Min = info_xstr.minThresh)
      xc_top = info_xstr.top 
      xc_max = info_xstr.maxThresh & xc_min = info_xstr.minThresh
      result = $
       BytScl( * info_xstr.image, Top = xc_top, Max = xc_max, Min = xc_min)
      ;;dummy = Routine_Names('xcontrast_result', result, Store = 2)
   END
   'savefg':BEGIN
      xc_max = info_xstr.maxThresh & xc_min = info_xstr.minThresh & mask = 2b
   END
   'savebg':BEGIN
      xc_max = info_xstr.maxThresh & xc_min = info_xstr.minThresh & mask = 1b
   END
   'savemissing':BEGIN
      xc_max = info_xstr.maxThresh & xc_min = info_xstr.minThresh & mask = 0b
   END
   'quit':BEGIN
      goto,fin
   END
ENDCASE
save, result, xc_top, xc_max, xc_min, mask, filename = 'x_result.sav'
fin:
...
END
;  of Xcontrast_ROI_done   *********************************************




PRO XCONTRAST_ROI_SAVEAS, event
...
;          BytScl( * info_xstr.image, Top = info_xstr.top,  $
;                  Max = info_xstr.maxThresh, Min = info_xstr.minThresh)
;         dummy = Routine_Names('xcontrast_image', displayImage, Store = 1)
;      ENDIF
;   END
ENDCASE
...
Widget_Control, event.top, Set_UValue = info_xstr, / No_Copy
END
; of Xcontrast_roi_saveas   *********************************************



PRO XCONTRAST_ROI_PROCESS_EVENTS, event
...
;; event handler for the draw w;idget to XCONTRAST_ROI_MOVELINE.
...
Widget_Control, event.id, Event_Pro = 'XCONTRAST_ROI_MOVELINE', $
...
Widget_Control, event.top, Set_UValue = info_xstr, / No_Copy
END
; of XCONTRAST_ROI_PROCESS_EVENTS *********************************************



PRO Xcontrast_ROI_MOVELINE, event
...
;; draw widget back to XCONTRAST_ROI_PROCESS_EVENTS.
...
IF thisEvent EQ 'UP' THEN BEGIN
   ...
   ;; to XCONTRAST_ROI_PROCESS_EVENTS, turn MOTION events OFF, and apply the
   ...
   ;; to XCONTRAST_ROI_PROCESS_EVENTS.
   Widget_Control, event.id, Draw_Motion_Events = 0, $
    Event_Pro = 'Xcontrast_Roi_Process_Events'
   ...
   CASE info_xstr.lineby OF
      'MIN': BEGIN
         ...
         PlotS, [info_xstr.maxThresh, info_xstr.maxThresh],  $
          [info_xstr.ymin, info_xstr.ymax], Color = info_xstr.maxColor, Thick = 3
      ...
      'MAX': BEGIN
         ...
         PlotS, [info_xstr.minThresh, info_xstr.minThresh],  $
          [info_xstr.ymin, info_xstr.ymax], Color = info_xstr.minColor, Thick = 3
   ...
   displayImage = BytScl( * info_xstr.image, Top = info_xstr.ncolors - 1,  $
                          Max = info_xstr.maxThresh, Min = info_xstr.minThresh)
   IF info_xstr.notify_pro NE "" THEN $
    Call_Procedure, $
    info_xstr.notify_pro, BytScl( * info_xstr.image, Top = info_xstr.top,  $
                             Max = info_xstr.maxThresh, Min = info_xstr.minThresh), $
    _Extra = info_xstr.extra
   ...
   IF NOT info_xstr.no_window THEN BEGIN
      IF Widget_Info(info_xstr.image_draw, / Valid_ID) THEN BEGIN
         WSet, info_xstr.windex
         TVLCT, info_xstr.r, info_xstr.g, info_xstr.b
         TVImage, displayImage, _Extra = info_xstr.extra
         IF info_xstr.notify_pro NE "" THEN Call_Procedure, info_xstr.notify_pro, $
      ...
      ENDIF ELSE BEGIN
         ...
         yoff = offsets[1]
         image_tlb = $
          Widget_Base(Row = 1, Group = event.top, Title = 'Contrast image', $
         ...
         XManager, 'xcontrast_roi_image', image_tlb, / No_Block
         Widget_Control, info_xstr.saveas, Sensitive = 1
      ENDELSE
   ENDIF

   ;; Update the pixmap with histogram with no threshold lines.
   Xcontrast_Roi_Histoplot, info_xstr.image, Background = info_xstr.backColor, $
...
CASE info_xstr.lineby OF
   'MIN': BEGIN
      cmax = Convert_Coord(info_xstr.maxThresh, 0, / Data, / To_Normal)
      PlotS, [info_xstr.maxthresh, info_xstr.maxthresh], [info_xstr.ymin, info_xstr.ymax],  $
       Color = info_xstr.maxColor, Thick = 3
      XYOuts, cmax[0], 0.90, / Normal, StrTrim(Fix(info_xstr.maxThresh), 2), $
   ...
   'MAX': BEGIN
      cmin = Convert_Coord(info_xstr.minThresh, 0, / Data, / To_Normal)
      PlotS, [info_xstr.minthresh, info_xstr.minthresh], [info_xstr.ymin, info_xstr.ymax],  $
       Color = info_xstr.minColor, Thick = 3
      XYOuts, cmin[0], 0.90, / Normal, StrTrim(Fix(info_xstr.minThresh), 2), $
...
Widget_Control, event.top, Set_UValue = info_xstr, / No_Copy
END
;; of XCONTRAST_ROI_MOVELINE **************************************


PRO XCONTRAST_ROI_MAXVALUE, event
;; Error Handling.
Catch, theError
IF theError NE 0 THEN BEGIN
   Catch, / Cancel
...
WSet, info_xstr.histo_wid
PlotS, [info_xstr.minThresh, info_xstr.minThresh], [!Y.Crange(0), !Y.Crange(1)], $
 Color = info_xstr.minColor, Thick = 3
PlotS, [info_xstr.maxThresh, info_xstr.maxThresh], [!Y.Crange(0), !Y.Crange(1)], $
 Color = info_xstr.maxColor, Thick = 3
cmax = Convert_Coord(info_xstr.maxThresh, 0, / Data, / To_Normal)
cmin = Convert_Coord(info_xstr.minThresh, 0, / Data, / To_Normal)
XYOuts, cmin[0], 0.90, / Normal, StrTrim(Fix(info_xstr.minThresh), 2), $
...
Widget_Control, event.top, Set_UValue = info_xstr, / No_Copy
END
;; of XCONTRAST_ROI_IMAGE_maxvalue ******************************************




PRO Xcontrast_ROI_CLEANUP, tlb
...
ENDIF
END
;; of xcontrast_roi_cleanup  ******************************************



PRO Xcontrast_ROI_QUIT, event
widget_Control, event.top, / Destroy
END
;; of XCONTRAST_ROI_QUIT ******************************************




PRO Xcontrast_ROI, theImage, Group_Leader = groupleader, NColors = ncolors, $
                   Max_Value = maxValue, Colortable = ctable, _EXTRA = extra, $
                   Notify_Pro = notify_pro, No_Window = no_window, $
                   XPos = xpos, YPos = ypos, Title = title, modal = modal, $
                   Notify_Obj = notify_obj, Block = block, Top = top, $
                   mspagroup = mspagroup, cancel = cancel
...
IF N_Elements(title) EQ 0 THEN title = 'Drag colorbars...'
...
histo_tlb = $
 Widget_Base(Title = title, XOffset = xpos, / column, $
             YOffset = ypos, tlb_frame_attr = 17);, / Base_Align_Center)
...
histo_draw = $
 Widget_Draw(histo_tlb, XSize = histXsize, YSize = histYsize, $
             Button_Events = 1, Event_Pro = 'Xcontrast_Roi_Process_Events')
controlID = $
 Widget_Button(toolbase, Value = 'Controls', $
               Event_Pro = 'Xcontrast_Roi_MaxValue', / menu)
; Widget_Button(menubaseID, Value = 'Controls', $
;               Event_Pro = 'Xcontrast_Roi_MaxValue')
dummy = $
 Widget_Button(toolbase, Value = 'Quit', uvalue = 'QUIT', $
               Event_Pro = 'Xcontrast_Roi_Done')
IF N_Elements(mspagroup) EQ 0 THEN BEGIN
   dummy = $
    Widget_Button(toolbase, Value = 'Accept', uvalue = 'accept', $
                  Event_Pro = 'Xcontrast_Roi_Done')
ENDIF ELSE BEGIN
   dummy = $
    Widget_Button(toolbase, Value = "Set2FG", uvalue = 'savefg', $
                  Event_Pro = 'Xcontrast_Roi_Done')
   dummy = $
    Widget_Button(toolbase, Value = "Set2BG", uvalue = 'savebg', $
                  Event_Pro = 'Xcontrast_Roi_Done')
   dummy = $
    Widget_Button(toolbase, Value = "Set2MISSING", uvalue = 'savemissing', $
                  Event_Pro = 'Xcontrast_Roi_Done')
ENDELSE
saveAs = $
 Widget_Button(controlID, Value = 'Save image as', $
               Event_Pro = "Xcontrast_Roi_SaveAs", / Menu)
...
;                        Event_Pro = 'Xcontrast_Roi_Quit', / Separator)
...
IF NOT Keyword_Set(no_window) THEN BEGIN
   ...
   ENDIF
   image_tlb = $ ;histo_tlb
    Widget_Base(Row = 1, Group_Leader = histo_tlb, Title = 'Contrast image', $
                XOffSet = xoff, YOffSet = yoff, TLB_frame_attr = 17);, $
;                 / modal) ;modal = keyword_set(modal))
   image_draw = $
    Widget_Draw(image_tlb, XSize = xsize, YSize = ysize, $
                Kill_Notify = 'Xcontrast_Roi_ImageWindowKilled', $
                UValue = [saveAs])
...
;; Start with 2% linear contrast on both ends.
...
minThresh = minVal + (0.0 * maxVal)
Xcontrast_Roi_Histoplot, image, Background = backColor, Color = drawColor, $
...
displayImage = BytScl( * image, Top = ncolors - 1, Max = maxThresh, $
                       Min = minThresh)
IF NOT Keyword_Set(no_window) THEN BEGIN
   WSet, windex
...
;        contrast_image:contrast_image, $ ;the output image if contrast applied
...
;        destroy_groupleader:destroy_groupleader, $
;        cancel:cancel, $
...
;   XManager, 'xcontrast_roi_image', image_tlb, No_Block = 1;0, Group = group ;1
;ENDIF
XManager, 'xcontrast_roi', histo_tlb, Group_leader = group, $
 No_Block = 1 - Keyword_Set(block), Cleanup = 'Xcontrast_Roi_Cleanup'
;XManager, 'xcontrast_roi', histo_tlb, Group = group, $
; No_Block = 1 - Keyword_Set(block), Cleanup = 'Xcontrast_Roi_Cleanup'
END
